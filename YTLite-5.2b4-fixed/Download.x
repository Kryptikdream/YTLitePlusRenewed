...
static NSArray *YouModPlayerResponsesForPlayer(YTPlayerViewController *player) {
    ...
    response = YouModObjectFromSelector(player, @selector(playerResponse));
    ...
    // Robust modern path: activeVideo -> singleVideo/videoData -> playbackData -> playerResponse
    id singleVideo = YouModObjectFromSelector(activeVideo, @selector(singleVideo)) ?: YouModObjectFromSelector(activeVideo, @selector(videoData));
    id playbackData = YouModObjectFromSelector(singleVideo, @selector(playbackData));
    id modernResponse = YouModObjectFromSelector(playbackData, @selector(playerResponse));
    if (modernResponse && ![responses containsObject:modernResponse]) [responses addObject:modernResponse];
...
static NSArray *YouModAdaptiveFormatObjectsForPlayer(YTPlayerViewController *player) {
    ...
    // Path 2: YTSingleVideoController → YTSingleVideo → MLVideo/playbackData → MLStreamingData
    id singleVideo = YouModObjectFromSelector(activeVideo, @selector(singleVideo)) ?: YouModObjectFromSelector(activeVideo, @selector(videoData));
    if (singleVideo) {
        id mlVideo = YouModObjectFromSelector(singleVideo, @selector(video));
        if (!mlVideo) {
            id playbackData = YouModObjectFromSelector(singleVideo, @selector(playbackData));
            mlVideo = YouModObjectFromSelector(playbackData, @selector(video));
        }
        if (mlVideo) {
            id mlStreamingData = YouModObjectFromSelector(mlVideo, @selector(streamingData));
            appendFormats(YouModObjectFromSelector(mlStreamingData, @selector(adaptiveStreams)));
        }
    }
...
static void YouModShowCaptionsSheet(YTPlayerViewController *player, UIViewController *presenter, UIView *sender) {
    ...
    for (id track in tracks) {
        ...
        if (baseURL.length == 0) baseURL = YouModStringFromSelector(track, @selector(baseUrl));
