...
static NSArray <YouModAudioOutputFormat *> *YouModAudioOutputFormats(void) {
    ...
    dispatch_once(&onceToken, ^{
        formats = @[
            ...
            YouModAudioOutputFormatMake(@"mp4", @"MP4", @"AAC audio in MP4 container", @"mp4", @[@"-map", @"0:a:0", @"-vn", @"-c:a", @"aac", @"-b:a", @"192k", @"-movflags", @"+faststart"], NO, YES),
        ];
    });
    return formats;
}
...
static void YouModShowSavedToFilesPopup(NSURL *fileURL, UIViewController *presenter) {
    ...
    if (![NSFileManager.defaultManager fileExistsAtPath:fileURL.path] || fileSize == 0) {
        YouModSendToast(@"Save to Files failed", presenter);
        return;
    }

    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"The video has succsesfully been saved to files"
                                                                   message:nil
...
- (void)completeWithFileURL:(NSURL *)fileURL isVideo:(BOOL)isVideo presenter:(UIViewController *)presenter {
    ...
    [self cleanupTemporaryFiles];
    YouModShowSavedToFilesPopup(fileURL, presenter);
}
...
static NSArray <NSDictionary *> *YouModRequestedVideoQualityRows(void) {
    return @[
        @{@"title": @"144p30", @"height": @144, @"fps": @30},
        @{@"title": @"240p30", @"height": @240, @"fps": @30},
        @{@"title": @"360p30", @"height": @360, @"fps": @30},
        @{@"title": @"720p30", @"height": @720, @"fps": @30},
        @{@"title": @"1080p30", @"height": @1080, @"fps": @30},
    ];
}

static YouModMediaFormat *YouModBestVideoFormatForRequestedQuality(NSArray <YouModMediaFormat *> *formats, NSInteger height, NSInteger fps) {
    YouModMediaFormat *best = nil;
    for (YouModMediaFormat *format in formats) {
        NSInteger formatHeight = YouModResolutionFromQuality(format.qualityLabel);
        NSInteger formatFPS = YouModNormalizedFPS(format.fps ?: YouModFPSFromQuality(format.qualityLabel));
        if (formatHeight != height) continue;
        if (formatFPS != 0 && formatFPS != fps) continue;
        ...
        BOOL formatMP4 = YouModFormatLooksMP4Family(format);
        BOOL bestMP4 = YouModFormatLooksMP4Family(best);
        if (formatMP4 != bestMP4) {
            if (formatMP4) best = format;
            continue;
        }
        if (format.contentLength > best.contentLength) best = format;
    }
    return best;
}

static NSString *YouModVideoRowSubtitle(YouModMediaFormat *videoFormat, YouModMediaFormat *audioFormat) {
    if (!videoFormat) return @"Unavailable";
    NSString *outputExtension = YouModMergedVideoOutputExtension(videoFormat, audioFormat).uppercaseString ?: @"MP4";
    unsigned long long combinedSize = videoFormat.contentLength + audioFormat.contentLength;
    NSString *size = YouModByteCount(combinedSize);
    return size.length ? [NSString stringWithFormat:@"%@ - %@", outputExtension, size] : [NSString stringWithFormat:@"%@ - size unknown", outputExtension];
}
...
static void YouModShowVideoQualitySheet(YTPlayerViewController *player, UIViewController *presenter, UIView *sender) {
    ...
    for (NSDictionary *quality in YouModRequestedVideoQualityRows()) {
        NSString *rowTitle = quality[@"title"];
        NSInteger height = [quality[@"height"] integerValue];
        NSInteger fps = [quality[@"fps"] integerValue];
        YouModMediaFormat *format = YouModBestVideoFormatForRequestedQuality(videoFormats, height, fps);
        NSString *subtitle = YouModVideoRowSubtitle(format, audioFormat);
        [items addObject:[YouModMenuItem itemWithTitle:rowTitle subtitle:subtitle icon:YouModIconImage(658) handler:^{
            if (!format) {
                YouModSendToast([NSString stringWithFormat:@"%@ is not available for this video", rowTitle], presenter);
                return;
            }
...
static void YouModShowAudioSheet(YTPlayerViewController *player, UIViewController *presenter, UIView *sender) {
    ...
    for (YouModAudioOutputFormat *format in YouModAudioOutputFormats()) {
        [items addObject:[YouModMenuItem itemWithTitle:format.title subtitle:YouModAudioOutputSubtitle(format) icon:YouModIconImage(21) handler:^{
            [[YouModDownloadCoordinator sharedCoordinator] startAudioDownloadWithAudioFormat:audioFormat fileName:title videoID:videoID outputFormat:format presenter:presenter];
        }]];
    }
    YouModPresentMenu(@"Download sound", items, presenter, sender);
}
